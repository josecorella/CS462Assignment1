...
class Graph():
    ...
    def prim(self, startNode):
        unreached = copy.deepcopy(self.edgeMap)
        mst = Graph()
        current = unreached.pop(startNode)
        reached = {}

        while unreached:
            ###  find the lowest-cost edge (u,v) such that
            ###  u is in reached v is in unreached
            min = sys.maxsize
            parent = None
            min_node = sys.maxsize
            for vertice in current:
                if int(vertice.weight) < min and vertice.dest not in reached:
                    min_node = int(vertice.dest)
                    min = int(vertice.weight)
                    parent = vertice.src
                reached[vertice.src] = current
            if parent:
                mst.addEdge(parent, str(min_node), str(min))
                current = unreached.pop(str(min_node))
            else:
                return mst
        return mst
